...
app.listen(PORT, () => {
  console.log(`🚀 SCARLA API server running on http://localhost:${PORT}`);
  console.log(`\n📝 API Endpoints:`);
  console.log(
    `   GET  /api/materials - Get all materials (HTML, CSS, JS, Java, Python, C#)`,
  );
  console.log(
    `   GET  /api/materials/:materialId - Get specific material with videos`,
  );
  console.log(`   GET  /api/quizzes - Get all quizzes`);
  console.log(`   GET  /api/quizzes/:quizId - Get specific quiz`);
  console.log(`   POST /api/quizzes/:quizId/submit - Submit quiz answers`);
